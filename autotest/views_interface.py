...
def show_edit_interface(req,edit_id,action):
    ...
    if(list(public_list_resp) != []):
        flag_resp = 'true'
    c = csrf(req)
    c.update({'edit_id': edit_id,'mode': mode,'body_format': body_format,'update_cookie': update_cookie,'interface_list_tmp': interface_list_tmp,'params':params,'head': head,'para_dict': para_dict,'para_str': para_str,'public_list_resp': public_list_resp,'flag_resp': flag_resp,'action': action,'env_paras': env_para,'product_alls':product_all,'product_name':product_name})
...
def start_interface_send(req):
    if req.method == "POST":
        ...
        n = 0
        while (n < 5):
            if (raw_data['interface_params'] != '' and raw_data['interface_params'] != None):
                body_format = 'FORM'
