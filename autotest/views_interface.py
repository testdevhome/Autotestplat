...
############################################
import json,traceback,re,copy,os,requests,hashlib,redis,random,time,string
...
def show_edit_interface(req,edit_id,action):
    ...
    if(list(public_list_resp) != []):
        flag_resp = 'true'
    c = csrf(req)
    c.update({'edit_id': edit_id,'mode': mode,'body_format': body_format,'update_cookie': update_cookie,'interface_list_tmp': interface_list_tmp,'head': head,'para_dict': para_dict,'para_str': para_str,'public_list_resp': public_list_resp,'flag_resp': flag_resp,'action': action,'env_paras': env_para,'product_alls':product_all,'product_name':product_name})
...
def save_edit_interface(req,edit_id):
    if req.method == "POST":
        ...
        if('Error' not in formated_dict and formated_dict != ''):
            ...
            AutotestplatInterfaceTestcase.objects.filter(id=edit_id).update(name=edit_name,url_host=edit_url_host,url=edit_url,charger=edit_creator,body=edit_body,head=edit_head,mode=edit_mode,product_id=edit_product_id,assert_keywords_old=assert_keywords_old,body_format=body_format)
