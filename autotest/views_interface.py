...
def save_edit_interface(req,edit_id):
    if req.method == "POST":
        ...
        formated_dict = req.POST.get('formated_dict','')
        edit_head_key = req.POST.get('head_key','')
        ...
        if (edit_params_key != ['']):
            ...
            edit_params =''
        if (edit_params != '' and edit_params != None):
            body_format = 'FORM'
        else:
            body_format = "JSON"
...
def start_interface_debug(req):
    if req.method == "POST":
        ...
        url = url_host+url
        print(url)
        print(raw_data['interface_head'])
        head=''
        # head = eval(raw_data['interface_head'])
        # for rec in head.keys():
        #     if(head[rec] in keyword_list1):
        #         head[rec] = public_dict[head[rec].replace('{','').replace('}','')]
        #     elif (head[rec] in keyword_list4):
        ...
        while (n < 5):
            ...
            if(raw_data['interface_head']!='' and raw_data['interface_head']!=None):
                body_format='FORM'
            else:
                body_format = 'JSON'  # raw_data['interface_format']
            try:
