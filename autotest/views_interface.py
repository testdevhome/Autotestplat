...
def getApiView(req):
    ...
    if AuthUser.objects.filter(username=username).first().is_superuser==1:
        interfaces = AutotestplatInterfaceTestcase.objects.filter(menu_module_id=0).order_by('-id').all()
        ...
        try:
            interfaces = AutotestplatInterfaceTestcase.objects.filter(product_id=user_product_id,menu_module_id=0).order_by('-id').all()
        except:
            interfaces = AutotestplatInterfaceTestcase.objects.filter(menu_module_id=0).order_by('-id').all()
...
def loadApiTestcaseTable(request,menu_module_id):
    ...
    #         user_product_id = AuthUser.objects.filter(username=username).first().last_name
    #     if user_product_id:
    #         items = AutotestplatInterfaceTestcase.objects.filter(Q(product_id=user_product_id)).values_list('id','name','mode','url', 'charger','product_id','menu_module_id').annotate(Count('id')).order_by('-id')
    #     else:
    #         items = AutotestplatInterfaceTestcase.objects.all().values_list('id','name','mode','url', 'charger','product_id','menu_module_id').annotate(Count('id')).order_by('-id')
    #
    # else:
    #     print(menu_module_id)
    if AuthUser.objects.filter(username=username).first().is_superuser == 1:
        items = AutotestplatInterfaceTestcase.objects.filter(menu_module_id=menu_module_id).values_list('id','name','mode','url', 'charger','product_id','menu_module_id').annotate(Count('id')).order_by('-id')
    else:
        user_product_id = AuthUser.objects.filter(username=username).first().last_name
        if user_product_id:
            items = AutotestplatInterfaceTestcase.objects.filter(Q(product_id=user_product_id),Q(menu_module_id=menu_module_id)).values_list('id','name','mode','url', 'charger','product_id','menu_module_id').annotate(Count('id')).order_by('-id')
        else:
            items = AutotestplatInterfaceTestcase.objects.filter(menu_module_id=menu_module_id).values_list('id','name','mode','url', 'charger','product_id','menu_module_id').annotate(Count('id')).order_by('-id')
    rst = []
    for item in items:
