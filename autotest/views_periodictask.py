...
############################################
import re,os,time,json,traceback,requests,autotest,string,random
...
def runPeriodictask(request,suit_id,task_id):
    runtask = 'api_autotest_task'+'('+str(suit_id.split('-')[0])+','+task_id+')'
    eval(runtask)
...
def loadPeriodicTask(request):
    ...
    if AuthUser.objects.filter(username=username).first().is_superuser == 1:
        ...
        testplans = AutotestplatTestplan.objects.filter(product_id=user_product_id).values_list().order_by('-id')
        items =[]
        for testplan in testplans:
            args1=str(testplan[0])+'-'+testplan[1]
            task = PeriodicTask.objects.filter(Q(args__icontains=args1)).values_list().order_by('-id')
            if task:
                items+=task
    rst = []
    for item in items:
...
def loadOptions(request):
    ...
    if AuthUser.objects.filter(username=username).first().is_superuser == 1:
        ...
        ids = AutotestplatTestplan.objects.filter(product_id=product_id).values('id').order_by('id').distinct()
    for id in ids:
...
report_id = ''
@app.task
def api_autotest_task(suit_id,task_id):
    try:
        global test_time
        ...
        for rec in interface_list:
            ...
            interfaceTestTask(case_list,test_time,response_time,report_id,task_id)
...
def interfaceTestTask(case_list,test_time,response_time,report_id,task_id):
    res_flags = []
    request_urls = []
    responses = []
    ...
    for case in case_list:
        try:
            ...
            result_info = AutotestplatTestplanInterfaceResult(report_id=report_id,task_id=task_id,suit_id=suit_id1,suit_interface_id=case_id,interface_name=interface_name,url=url,body=body,mode=mode,assert_keywords_old=assert_keywords_old,response_time=response_time,task_mode="定时任务",response=response,result=result1,date_time=test_time)
            result_info.save()
            return result1
        except Exception:
            error_info = traceback.format_exc()
