...
def save_edit_testplan(request):
    if request.method == "POST":
        ...
        AutotestplatTestplan.objects.filter(id=suit_info[0]).update(suit_name=suit_info[1],interface_name=interfaces,url_host=suit_info[2], interface_name_display=interfaces1,product_id=product_id)
    ...
    if req.method == "GET":
        ...
        if(id2 == ''):
            try:
                ...
                public_list_resp = AutotestplatParameter.objects.filter(module_id=int(id1)).exclude(product_id='testplan')
        ...
        if(id1 != 'new'):
            ...
            if(resp_info != []):
                ...
                for i in range(len(resp_edit_name)):
                    AutotestplatParameter.objects.filter(module_id=id1).update(name=resp_edit_name[i],keywords=resp_edit_keywords[i],value='',left=resp_edit_left[i],right=resp_edit_right[i],index=resp_edit_index[i],product_id="testplan",type='res')
            ...
            if(resp_info != []):
                ...
                for i in range(len(resp_edit_name)):
                    resp_info1 = AutotestplatParameter(name=resp_edit_name[i],keywords=resp_edit_keywords[i],value='',left=resp_edit_left[i],right=resp_edit_right[i],index=resp_edit_index[i],product_id="testplan",module_id=max_id,type='res')
                    resp_info1.save()
            max_id = AutotestplatTestplanInterface.objects.order_by('-id')[0].id
...
def interfaceTestTask(case_list,test_time,response_time,report_id):
    ...
    for case in case_list:
        try:
            ...
            result_info = AutotestplatTestplanInterfaceResult(report_id=report_id,product_id=product_id,suit_id=suit_id1,suit_interface_id=case_id,interface_name=interface_name,url=url,body=body,mode=mode,assert_keywords_old=assert_keywords_old,response_time=response_time,task_mode="定时任务",response=response,result=result1,date_time=test_time)
            result_info.save()
            return result1
        except Exception:
            error_info = traceback.format_exc()
