...
def loadReport(request):
    ...
    if AuthUser.objects.filter(username=username).first().is_superuser == 1:
        items = AutotestplatTestplanInterfaceResult.objects.all().values_list('report_id','product_id', 'suit_id', 'date_time','task_mode','pass_pers').annotate(Count('id'))
    else:
        ...
        items = []
        for suit in suits:
            result = AutotestplatTestplanInterfaceResult.objects.filter(suit_id=suit.id).values_list('report_id','product_id', 'suit_id', 'date_time','task_mode','pass_pers').annotate(Count('id'))
            if result:
                items +=result
    rst = []
    for item in items:
